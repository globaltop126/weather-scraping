...
def extract_monthly_data(data):
    ...
    for i in range(num_days):
        ...
        wanted_keys = ['date', 'maxtempC', 'weatherDesc']  # The keys you want
        subset_d = dict((k, d[k]) for k in wanted_keys if k in d)
        subset_d['Celsius degrees'] = subset_d.pop('maxtempC')
        ...
        col_to_keep = ['date', 'Celsius degrees', 'weatherDesc']
        df = df[col_to_keep]
        df['Weather condition'] = df.pop('weatherDesc')
...
def retrieve_this_location(api_key, location_list, start_date, end_date, frequency, response_cache_path):
...
def retrieve_hist_data(api_key, location_list, start_date, end_date, frequency, location_label=False, export_csv=True,
    ...
    df_this_city = retrieve_this_location(api_key, location_list, start_date, end_date, frequency, response_cache_path)
    ...
    if (export_csv == True):
        df_this_city.to_csv('./Chile Weather.csv', header=True, index=False)
